...
import { LayoutDashboard, CheckSquare, MessageCircle, Award, LogOut, Menu, User } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Certificate', icon: Award, path: '/student/certificate' },
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <aside
      className={cn(
        ...
        mobile ? 'w-full' : 'w-64 min-w-[256px]'
      )}
      ...
      <div className="px-6 py-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-white/40 text-xs truncate">{user?.domain}</p>
          </div>
        </div>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              style={active ? { background: '#FF4D00', color: '#fff' } : {}}
            >
              <item.icon size={18} />
              {item.label}
            </Link>
